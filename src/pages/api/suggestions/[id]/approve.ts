...
export default async function handler(
  ...
  if (!requireOwner(user)) {
    return res.status(403).json({ error: "Only owners can approve suggestions" });
  }
  ...
  if (!hasServiceRole()) {
    return res.status(200).json({ status: "approved", demo: true });
  }
  ...
  await writeAuditLog(req, "suggestion.approved", user.email, {
